        ...
        <f7-list class="no-hairlines-between">
          <div v-for="(filter, type) in filters" :key="type">
            ...
            <f7-list-item class="padding-bottom">
              <div class="chip-wrap">
                <f7-chip v-for="(label, value) in filter.options" :key="value" :text="label" media-bg-color="blue"
                         :color="isFilteredBy(type, value) ? 'blue' : ''"
                         style="margin-right: 6px; cursor: pointer;"
                         @click="toggleFilter(type, value)">
                  <f7-icon v-if="isFilteredBy(type, value)" slot="media" ios="f7:checkmark_circle_fill" md="material:check_circle" aurora="f7:checkmark_circle_fill" />
                </f7-chip>
              </div>
            </f7-list-item>
          </div>
...
export default {
  ...
  methods: {
    ...
    toggleFilter (type, value) {
      ...
      // Create a new Set and use $set to ensure reactivity in @vue2
      // This can probably be removed in Vue3
      this.$set(this.selected, type, new Set(selections))
    ...
    resetFilters () {
      Object.keys(this.selected).forEach(type => {
        // @vue2 does not track changes to Sets, so we need to assign it to a new Set
        // It can be changed to `this.selected[type].clear()` in Vue3
        this.$set(this.selected, type, new Set())
      })
