...
export default {
  ...
  computed: {
    ...
    className() {
      ...
      const property = semantics.config && semantics.config.relatesTo ? semantics.config.relatesTo : null
      return (
        this.model.class.substring(this.model.class.lastIndexOf('_') + 1) + (property ? ' (' + property.replace('Property_', '') + ')' : '')
      )
    },
    itemSelected() {
