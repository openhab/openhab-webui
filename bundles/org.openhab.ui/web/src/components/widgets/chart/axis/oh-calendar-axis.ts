...
const calendarAxis: AxisComponent = {
  get(context, component, startTime, endTime) {
    ...
    if (!calendar.monthLabel) calendar.monthLabel = ({} as CalendarOption['monthLabel'])!
    ...
    if (useUIOptionsStore().darkMode === 'dark') {
      if (!calendar.itemStyle) calendar.itemStyle = ({} as CalendarOption['itemStyle'])!
      ...
      if (!calendar.splitLine) calendar.splitLine = ({} as CalendarOption['splitLine'])!
