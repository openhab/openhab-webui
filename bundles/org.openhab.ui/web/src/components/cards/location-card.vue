<template>
  <model-card type="location" :context="context" :element="element" header-height="200px">
    <template v-slot:glance>
      ...
      <div class="location-stats margin-top" :class="config.invertText ? 'invert-text' : ''">
        <status-badge :store="context.store" :element="element" :type="badgeType" :key="badgeType" :invert-color="config.invertText"
          v-for="badgeType in ['lights', 'windows', 'doors', 'garagedoors', 'blinds', 'presence', 'lock', 'climate', 'screens', 'projectors', 'speakers']" />
      </div>
      <div class="location-stats margin-top-half">
        <measurement-badge :store="context.store" :element="element" :type="badgeType" :key="badgeType" :invert-color="config.invertText"
          v-for="badgeType in ['temperature', 'humidity', 'luminance']" />
      </div>
    </template>
