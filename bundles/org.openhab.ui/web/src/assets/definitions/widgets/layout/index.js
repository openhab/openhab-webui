...
export function OhGridColDefinition() {
  const columnOptions = [5, 10, 15, 20, 25, 30, 33, 35, 40, 45, 50, 55, 60, 65, 66, 70, 75, 80, 85, 90, 95, 100].map((c) => {
    return {
      value: c.toString(),
      label: `${c} %`
    }
  ...
  return new WidgetDefinition('oh-grid-col', 'Layout Grid Column', 'A column in a grid layout').params([
...
export function OhMasonryDefinition() {
  return new WidgetDefinition('oh-masonry', 'Masonry Layout', 'Arranges widgets automatically depending on the screen size').params([
...
export function OhCanvasItemDefinition() {
  return new WidgetDefinition('oh-canvas-item', 'Canvas Item', 'Specific attributes to display widgets on a canvas.').paramGroup(
...
export function OhCanvasLayoutDefinition() {
  return new WidgetDefinition(
    ...
    .paramGroup(pg('layout', 'Layout Settings'), [pn('grid', 'Grid size', 'Grid size in pixels used to snap content (default 20)')])
    .paramGroup(pg('screenSettings', 'Screen Settings'), [
