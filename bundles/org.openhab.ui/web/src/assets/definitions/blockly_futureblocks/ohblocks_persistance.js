...
export default function defineOHBlocks_Persistance (f7) {
  ...
  Blockly.JavaScript['oh_setpersist'] = function (block) {
    ...
    let code = 'this.' + varname + ' = (this.' + varname + ' === undefined) ? ' + value + ' : this.' + varname + ';\n'
  ...
  Blockly.JavaScript['oh_persistanceLastUpdate'] = function (block) {
    ...
    // TODO: Assemble JavaScript into code variable.
    let code = persistenceExtensions + '.previousState(itemRegistry.getItem(' + itemName + '), true, "rrd4j")'
    return [code, 0]
  }
}
