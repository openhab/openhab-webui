...
export default function (f7, sinks, voices) {
  ...
  javascriptGenerator.forBlock['oh_playmedia_sink'] = function (block) {
    ...
    let sinkName = valueToCode(block, 'sinkName', javascriptGenerator.ORDER_ATOMIC)
      .replace('(', '')
      .replace(/[()]/g, '')
  ...
  javascriptGenerator.forBlock['oh_say'] = function (block) {
    ...
    const voiceName = valueToCode(block, 'voice', javascriptGenerator.ORDER_ATOMIC)
      .replace('(', '')
      .replace(/[()]/g, '')
    const deviceSink = valueToCode(block, 'deviceSink', javascriptGenerator.ORDER_ATOMIC)
      .replace('(', '')
      .replace(/[()]/g, '')
