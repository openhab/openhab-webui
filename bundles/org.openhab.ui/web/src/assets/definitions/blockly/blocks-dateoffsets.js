...
export default function (f7) {
  ...
  Blockly.Blocks['oh_zdt_create'] = {
    ...
    updateShape_: function () {
      this.appendValueInput('year').setCheck('Number').appendField('datetime with date')
      this.appendValueInput('month').setCheck('Number').appendField('-')
      this.appendValueInput('day').setCheck('Number').appendField('-')
      this.appendValueInput('hour').setCheck('Number').appendField('and time')
  ...
  Blockly.Blocks['oh_zdt_fromItem'] = {
    init: function () {
      this.appendValueInput('itemName').appendField('datetime from item').setCheck(['String', 'oh_item'])
      this.setOutput(true, 'ZonedDateTime')
      this.setColour(70)
  ...
  Blockly.Blocks['oh_zdt_temporal_unit_input'] = {
    ...
    init: function () {
      this.appendDummyInput().appendField(new Blockly.FieldImage(nextImage, 15, 15, undefined, this.onClick), 'NEXT')
  ...
  javascriptGenerator.forBlock['oh_zdt_amend'] = function (block) {
    ...
    if (operationBlock) {
      ...
      if (millisChanged || microsChanged || nanosChanged) {
        nanoCode += !millisChanged
          ? `(${baseZdt}.get(time.ChronoField.NANO_OF_SECOND) / 1000000 | 0) * 1000000  + `
          : `${millis}  * 1000000 +`
        nanoCode += !microsChanged
          ? `((${baseZdt}.get(time.ChronoField.NANO_OF_SECOND) / 1000) % 1000 | 0) * 1000 + `
          : `${micros} * 1000 +`
  ...
  javascriptGenerator.forBlock['oh_get_zdt_part'] = function (block) {
    ...
    const op = new Map([
      ...
      ['getMinute', 'minute'],
      ['getSecond', 'second'],
      ['getNanoTotal', 'get(time.ChronoField.NANO_OF_SECOND)']
    ])
    switch (temporalPart) {
      case 'getMilli':
