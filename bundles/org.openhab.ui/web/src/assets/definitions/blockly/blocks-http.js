...
export default function (f7) {
  ...
  Blockly.Blocks['oh_httprequest'] = {
    ...
    updateShape: function (hasTimeout, hasHeader, hasQuery) {
      ...
      if (hasTimeout) {
        if (!timeoutInput) {
          timeoutInput = this.appendValueInput('timeoutInput').setCheck('Number').appendField('with Timeout (ms)')
      ...
      if (hasHeader) {
        if (!headerInput) {
          headerInput = this.appendValueInput('requestHeader').setCheck('Dictionary').appendField('with Headers')
      ...
      if (hasQuery) {
        if (!queryInput) {
          queryInput = this.appendValueInput('query').setCheck('Dictionary').appendField('with Query')
