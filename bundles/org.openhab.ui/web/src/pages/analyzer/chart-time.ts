...
const timeCoordSystem: CoordSystem = {
  ...
  initSeries(
    ...
    if (options.type === SeriesType.state) {
      ...
      options.type === SeriesType.area ||
      options.type === SeriesType.bar
    ) {
  ...
  getChartPage(
    ...
    let categoryGridSize =
      timeCoordSettings.categoryAxisValues.length * GRID_CONFIG.CATEGORY_HEIGHT_PER_ITEM_PERCENT
    ...
    if (valueGrid) {
      page.slots.grid.push({
        ...
        config: {
          includeLabels: true,
          bottom: categoryGrid
            ? `${categoryGridSize + GRID_CONFIG.GRID_PADDING_PERCENT}%`
            : GRID_CONFIG.DEFAULT_BOTTOM_MARGIN_PIX
        }
      })
