...
export default {
  ...
  methods: {
    ...
    async updateItems (itemNames : Array<string>) {
      ...
      this.showChart = false
      const promises = itemNames.map((n) => api.getItemByName({ path: { itemname : n } }).then((data) => data as api.EnrichedItem))
    ...
    doSavePage (uid : string, label: string, overwrite : boolean = false) {
      ...
      const promise = (!overwrite)
        ? api.addUiComponentToNamespace({ path: { namespace: 'ui:page' }, body: chartPage })
        : api.updateUiComponentInNamespace({ path: { namespace: 'ui:page', componentUID: uid }, body: chartPage })
      promise.then((data) => {
        if (overwrite) {
          f7.toast.create({
