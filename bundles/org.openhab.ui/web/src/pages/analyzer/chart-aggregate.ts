...
const DIMENSION_MAP: Partial<
  Record<
    ChartType,
    [OhAggregateSeries.Dimension, OhCategoryAxis.CategoryType, OhAggregateSeries.Dimension]
  >
> = {
  // dimension1, category2, dimension2
  [ChartType.day]: [
    OhAggregateSeries.Dimension.hour,
    OhCategoryAxis.CategoryType.hour,
    OhAggregateSeries.Dimension.minute
  ],
  [ChartType.week]: [
    OhAggregateSeries.Dimension.weekday,
    OhCategoryAxis.CategoryType.day,
    OhAggregateSeries.Dimension.hour
  ],
  [ChartType.isoWeek]: [
    OhAggregateSeries.Dimension.isoWeekday,
    OhCategoryAxis.CategoryType.day,
    OhAggregateSeries.Dimension.hour
  ],
  [ChartType.month]: [
    OhAggregateSeries.Dimension.date,
    OhCategoryAxis.CategoryType.day,
    OhAggregateSeries.Dimension.hour
  ],
  [ChartType.year]: [
    OhAggregateSeries.Dimension.month,
    OhCategoryAxis.CategoryType.month,
    OhAggregateSeries.Dimension.date
  ]
}
...
const aggregateCoordSystem: CoordSystem = {
    ...
    return {
      ...
      chartType:
        coordSettings && coordSettings.chartType && typeOptions.includes(coordSettings.chartType)
          ...
          : ChartType.day,
      orientation:
        coordSettings && coordSettings.orientation ? coordSettings.orientation : Orient.horizontal,
  ...
  initSeries(
    ...
    if (
      ...
      options.aggregation =
        seriesOptions.aggregation || OhAggregateSeries.AggregationFunction.average
      ...
      if (aggregateCoordSettings.dimensions === 2) {
        ...
        options.type =
          seriesOptions.type && options.typeOptions.includes(seriesOptions.type)
            ? seriesOptions.type
            : SeriesType.bar
  ...
  getChartPage(
    ...
    const categoryType =
      aggregateCoordSettings.chartType === ChartType.isoWeek
        ? OhCategoryAxis.CategoryType.week
        : (aggregateCoordSettings.chartType as unknown as OhCategoryAxis.CategoryType)
    ...
    const dims = DIMENSION_MAP[aggregateCoordSettings.chartType] ?? [
      OhAggregateSeries.Dimension.hour,
      OhCategoryAxis.CategoryType.hour,
      OhAggregateSeries.Dimension.minute
    ]
    ...
    page.slots.series = items
      .filter(
        item =>
          item.type.startsWith('Number') || item.type === 'Dimmer' || item.type === 'Rollershutter'
      )
      .map((item: Item) => {
        ...
        return {
          ...
          config: {
            ...
            item: item.name,
            xAxisIndex:
              aggregateCoordSettings.orientation === Orient.vertical && !dimension2
            ...
            yAxisIndex:
              aggregateCoordSettings.orientation !== Orient.vertical && !dimension2
                ...
                : 0,
            type: dimension2
              ? OhAggregateSeries.Type.heatmap
              : seriesOptions.type === SeriesType.bar
                ? OhAggregateSeries.Type.bar
                : OhAggregateSeries.Type.line,
            dimension1,
            dimension2,
