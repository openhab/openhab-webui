...
const aggregateCoordSystem: CoordSystem = {
  initCoordSystem(coordSettings?: Partial<AggregateCoordSettings>): AggregateCoordSettings {
    ...
    return {
      ...
      typeOptions,
      chartType:
        coordSettings && coordSettings.chartType && typeOptions.includes(coordSettings.chartType) ? coordSettings.chartType : ChartType.day,
  ...
  getChartPage(
    ...
    items: (api.EnrichedItem | api.GroupItem)[]
  ): api.RootUiComponent {
    const aggregateCoordSettings = coordSettings as AggregateCoordSettings
    ...
    page.slots.series = items
      ...
      .map((item: api.EnrichedItem | api.GroupItem) => {
