<template>
        ...
        <f7-col>
          ...
          <f7-list media-list swipeout>
            <f7-list-item
              ...
              <f7-swipeout-actions v-if="editable" right>
                <f7-swipeout-button
                  @click.stop="(ev: MouseEvent) => deleteFilterOrStrategy(ev, 'cronStrategies', index)"
          ...
          <f7-list v-for="ft in Object.values(FilterTypes)" :key="ft.name" :media-list="editable" swipeout>
            <f7-list-item
              ...
              <f7-swipeout-actions v-if="editable" right>
                <f7-swipeout-button
                  @click.stop="(ev: MouseEvent) => deleteFilterOrStrategy(ev, ft.name as FilterTypeName, index)"
                  style="background-color: var(--f7-swipeout-delete-button-bg-color)">
                  Delete
                </f7-swipeout-button>
              </f7-swipeout-actions>
  ...
  <cron-strategy-popup
    ...
    :cronNameList="localPersistence.cronStrategies.map((cs) => cs.name)"
  ...
  <filter-popup
    ...
    :filter-name-list="allFilterNames"
...
import { FilterTypes, type Filter, FilterTypeName } from '@/assets/definitions/persistence'
...
const allFilterNames = computed(() => {
  return Object.values(FilterTypeName)
    .flatMap((ft) => localPersistence.value[ft])
    .map((f) => f.name)
})
...
async function deleteFilterOrStrategy(ev: MouseEvent, type: FilterTypeName | 'cronStrategies', index: number) {
  if (!props.editable) return
  const sName = localPersistence.value[type][index]?.name
  if (!sName) return
  // Check if strategy is used in configurations
  if (_isFilterOrStrategyUsed(type, sName)) {
    if (
      await showConfirmDialog(
        `${type === 'cronStrategies' ? 'Strategy' : 'Filter'} used in configuration(s), delete anyway?`,
        `Delete ${type === 'cronStrategies' ? 'Persistence Strategy' : 'Persistence Filter'}`
      )
    ) {
      _deleteModule(ev, type, index)
      _removeFilterOrStrategyFromConfiguration(type, sName)
    }
  } else {
    _deleteModule(ev, type, index)
  }
...
function _removeFilterOrStrategyFromConfiguration(type: 'cronStrategies' | FilterTypeName, name: string) {
  localPersistence.value.configs.forEach((cfg) => {
    if (type === 'cronStrategies') {
      const sIndex = cfg.strategies?.findIndex((s) => s === name) ?? -1
      if (sIndex >= 0 && cfg.strategies) cfg.strategies.splice(sIndex, 1)
    } else {
      const fIndex = cfg.filters?.findIndex((f) => f === name) ?? -1
      if (fIndex >= 0 && cfg.filters) cfg.filters.splice(fIndex, 1)
    }
  })
}

function _isFilterOrStrategyUsed(type: 'cronStrategies' | FilterTypeName, name: string | undefined) {
  return (
    localPersistence.value.configs?.findIndex((cfg) =>
      type === 'cronStrategies' ? cfg.strategies?.findIndex((cs) => cs === name) >= 0 : cfg.filters?.findIndex((f) => f === name) >= 0
    ) >= 0
  )
}
