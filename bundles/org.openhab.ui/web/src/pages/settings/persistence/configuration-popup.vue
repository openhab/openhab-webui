...
const groupItems = computed({
  get () {
    return localItemConfiguration.value.items.filter((i) => i.length > 1 && !i.startsWith('!') && i.endsWith('*')).map((i) => i.slice(0, -1))
  },
  set (newGroupItems) {
    localItemConfiguration.value.items = itemConfig(allItemsSelected.value, newGroupItems.sort((a, b) => a.localeCompare(b)), items.value, excludeGroupItems.value, excludeItems.value)
  }
})
...
const items = computed({
  ...
  set (newItems) {
    localItemConfiguration.value.items = itemConfig(allItemsSelected.value, groupItems.value, newItems.sort((a, b) => a.localeCompare(b)), excludeGroupItems.value, excludeItems.value)
  }
})
...
const excludeGroupItems = computed({
  ...
  set (newExcludeGroupItems) {
    localItemConfiguration.value.items = itemConfig(allItemsSelected.value, groupItems.value, items.value, newExcludeGroupItems.sort((a, b) => a.localeCompare(b)), excludeItems.value)
  }
})
...
const excludeItems = computed({
  ...
  set (newExcludeItems) {
    localItemConfiguration.value.items = itemConfig(allItemsSelected.value, groupItems.value, items.value, excludeGroupItems.value, newExcludeItems.sort((a, b) => a.localeCompare(b)))
  }
})
...
const allItemsSelected = computed({
  ...
  set (newAllItemsSelected) {
    localItemConfiguration.value.items = itemConfig(newAllItemsSelected, groupItems.value, items.value, excludeGroupItems.value, excludeItems.value)
  }
})
