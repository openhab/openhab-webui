...
async function loadPersistence(serviceId: string) : Promise<boolean> {
  ...
  suggestedStrategies.value = await api.getPersistenceServiceStrategySuggestions({ serviceId }).then((suggestions) => {
    return suggestions || []
  }).catch(() => {
...
async function deletePersistence () {
  ...
  if(!await showConfirmDialog(
    ...
    'Delete persistence configuration')) return
...
function fromYaml () {
  ...
  persistence.value.cronStrategies = 'cronStrategies' in updatedPersistence ? updatedPersistence.cronStrategies as api.PersistenceCronStrategy[] : []
  persistence.value[FilterTypeName.EqualsFilters] = FilterTypeName.EqualsFilters in updatedPersistence ? updatedPersistence[FilterTypeName.EqualsFilters] as api.PersistenceFilter[] : []
  persistence.value[FilterTypeName.TimeFilters] = FilterTypeName.TimeFilters in updatedPersistence ? updatedPersistence[FilterTypeName.TimeFilters] as api.PersistenceFilter[] : []
  persistence.value[FilterTypeName.ThresholdFilters] = FilterTypeName.ThresholdFilters in updatedPersistence ? updatedPersistence[FilterTypeName.ThresholdFilters] as api.PersistenceFilter[] : []
  persistence.value[FilterTypeName.IncludeFilters] = FilterTypeName.IncludeFilters in updatedPersistence ? updatedPersistence[FilterTypeName.IncludeFilters] as api.PersistenceFilter[] : []
