  ...
  <f7-block class="padding-vertical no-padding-horizontal">
    <f7-col>
      <f7-list class="no-margin" inline-labels no-hairlines-md>
        <f7-list-input v-if="createMode" label="Channel ID" type="text" placeholder="Required" :value="channel.id"
        ...
        <f7-list-item v-else :disabled="disabled" media-item class="channel-item" title="Channel UID">
          <div slot="subtitle">
            {{ channel.uid }}
            <clipboard-icon :value="channel.uid" tooltip="Copy UID" style="pointer-events: initial !important" />
          </div>
        </f7-list-item>
