...
export default {
  ...
  methods: {
    ...
    resetFilter () {
      this.$set(this, 'excludedUids', new Set()) // Ensure reactivity in @vue2. Just call clear in vue3
      if (this.searchQuery) {
        this.reapplySearch()
      } else {
        this.$refs.itemsList.f7VirtualList.resetFilter()
      }
    },
    applyFilter () {
      ...
      this.$set(this, 'excludedUids', new Set(this.excludedUids)) // Ensure reactivity in @vue2
      ...
      if (this.searchQuery) {
        ...
        this.$refs.itemsList.f7VirtualList.filterItems(filteredIndexes)
      } else {
        this.$refs.itemsList.f7VirtualList.resetFilter()
      }
    },
    ...
    updateListedItems () {
      this.$nextTick(() => {
        this.listedItems = this.$refs.itemsList.f7VirtualList.filteredItems || this.$refs.itemsList.f7VirtualList.items || []
      })
    }
  },
  watch: {
    listedItemsDependencies () {
  ...
  computed: {
    ...
    listedItemsDependencies () {
      return [this.ready, this.searchQuery, this.excludedUids]
    },
    allSelected () {
