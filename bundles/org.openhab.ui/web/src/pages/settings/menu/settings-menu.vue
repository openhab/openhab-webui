  ...
  methods: {
    loadMenu () {
      ...
      if (useRuntimeStore().apiEndpoint('addons')) {
        api.getAddons({ query: { serviceId: 'all' } }).then((data) => {
    ...
    loadCounters () {
      ...
      if (useRuntimeStore().apiEndpoint('links'))
        api.getOrphanLinks().then((data) => {
          ...
          console.error('Error loading orphan links:', error)
        })
      ...
      if (useRuntimeStore().apiEndpoint('items'))
        api.getSemanticsHealth().then((data) => {
          ...
          console.error('Error loading semantics health:', error)
        })
      ...
      if (useRuntimeStore().apiEndpoint('persistence'))
        api.getPersistenceHealth().then((data) => {
          ...
          console.error('Error loading persistence health:', error)
        })

      if (useRuntimeStore().apiEndpoint('inbox'))
        api.getDiscoveredInboxItems({ query: { includeIgnored: false } }).then((data) => {
      ...
      if (useRuntimeStore().apiEndpoint('things'))
        api.getThings({ query: { staticDataOnly: true } }).then((data) => {
      ...
      if (useRuntimeStore().apiEndpoint('items'))
        api.getItems({ query: { staticDataOnly: true } }).then((data) => {
      ...
      if (useRuntimeStore().apiEndpoint('ui'))
        api.getRegisteredUiComponentsInNamespace({ path: { namespace: 'system:sitemap' }}).then((data) => {
      ...
      if (useRuntimeStore().apiEndpoint('rules'))
        api.getRules({ query: { staticDataOnly: true } }).then((data) => {
