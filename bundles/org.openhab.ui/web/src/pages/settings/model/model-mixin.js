...
export default {
  ...
  methods: {
          ...
          if (up) {
            item.parents
              ?.filter(p => {
                ...
                return (
                  p.name === semanticsConfig.hasLocation ||
                  p.name === semanticsConfig.isPartOf ||
                  p.name === semanticsConfig.isPointOf
                )
              })
        ...
        .then(data => {
          ...
          if (this.includeNonSemantic) {
            this.rootGroups = this.items
              .filter(
                i => i.type === 'Group' && !i.metadata?.semantics && i.groupNames.length === 0
              )
              .map(this.modelItem)
              .sort(compareModelItems)
    ...
    expandChild(child) {
      if (child.opened) {
        this.saveExpandedChild(child)
      } else {
        this.saveCollapsedChild(child)
      }
    },
    saveExpanded() {
    ...
    saveExpandedChild(child) {
      if (child.opened && !this.expandedTreeviewItems.includes(child.item.name)) {
    ...
    saveCollapsedChild(child) {
      if (!child.opened) {
        this.expandedTreeviewItems = this.expandedTreeviewItems.filter(
          item => item !== child.item.name
        )
      }
    },
    restoreExpanded() {
    ...
    restoreExpandedChild(child, parentClosed) {
      if (parentClosed) {
        ...
        child.opened = this.expanded || this.expandedTreeviewItems.includes(child.item.name)
      }
      Object.values(child.children)
        .flat()
        .forEach(c => {
