...
export const useThingEditStore = defineStore('thingEditStore', () => {
  ...
  const hasLinkedItems = computed(() =>
    thing.value?.channels?.find((c: Channel) => c.linkedItems?.length)
  )
  ...
  async function loadConfigDescriptions(thingUID: string): Promise<void> {
    try {
      const data: ConfigDescriptionResponse = await api.get(
        '/rest/config-descriptions/thing:' + thingUID
      )
      configDescriptions.value = data
    } catch (e: any) {
      console.debug(
        'No specific config description available for this thing, using config description from thing type instead.'
      )
