...
function _compareObjects(
  o1: ModelItem | { item: ModelItem },
  o2: ModelItem | { item: ModelItem }
): number {
...
export const useModelStore = defineStore('model', () => {
  ...
  function getSemanticModelElement(
    key: string,
    type: string
  ): LocationModelCard | EquipmentModelCard | PropertyModelCard | null {
      ...
      .then((data: Item[]) => {
        ...
        modelItems.forEach(item => {
          if (item.metadata && item.metadata.semantics)
            buildPathInModel(item, modelItems, filteredItems)
        })
        ...
        equipment.value = Object.keys(equipmentStruct)
          .sort((a: string, b: string) =>
            (i18n.global as Composer).t(a).localeCompare((i18n.global as Composer).t(b))
          )
